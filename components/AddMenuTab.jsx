...
function AddMenuTab({ currentMenu, editingMenu, userCode, onSuccess }) {
  ...
  const handleSubmit = (e) => {
    ...
    const url = isEdit ? `${API_BASE}/api/menus/${editingMenu.id}` : `${API_BASE}/api/menus/current`;
    ...
    fetch(url, {
      ...
      .catch((err) => {
        ...
        setError(
          isNetworkError
            ? "Could not reach the server. Start the backend (e.g. python3 -m uvicorn backend.main:app --host 0.0.0.0 --port 8001). If it is running, try opening the app from http://127.0.0.1:5173 and ensure the backend is at http://127.0.0.1:8001."
